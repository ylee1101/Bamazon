	...
	inquirer.prompt([
		{
			type: 'input',
			name: 'id',
			message: 'Enter the ID Number of item',
		...
		connection.query(queryStr, {id: item}, function(err, data) {
			...
			if (data.length === 0) {
				...
				if (quantity <= productData.stockQty) {
					console.log('Congratulations, the product you requested is in stock! Placing order!');

					// Construct the updating query string
					var updateQueryStr = 'UPDATE products SET stockQty = ' + (productData.stockQty - quantity) + ' WHERE id = ' + item;
